...
from model.language_model.volcano_mistral import QuietVolCanoMistralForCausalLM, VolCanoMistralConfig
from transformers import LlamaTokenizer, AutoTokenizer
import json 
import time
# from eval_helpers import  compute_metrics
...
# print(f"Distributed training enabled? {accelerator.distributed_type != 'NO'}")
...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    output_dir: str = field(default="/inspire/hdd/ws-f4d69b29-e0a5-44e6-bd92-acf4de9990f0/public-project/zhangwanlin-240108540162/VoCoT/output")
    ...
    # output_dir=root_prefix + f"cache/quietstar/{run_id}",   ###重复
    # learning_rate=1e-6,   ###重复
    # optim="adamw_torch_fused" if torch.cuda.is_available() else "adamw_torch",  ###重复
    # per_device_train_batch_size=batch_size, ###重复
    # per_device_eval_batch_size=batch_size, ###重复
    # gradient_accumulation_steps=global_gradient_accumulation_steps,    ###重复
    max_grad_norm=1.0,
    max_steps=100000,
    # warmup_steps=20,    ###重复
...
def main(args):
    ...
    if model_args.use_mistral:
        ...
        model_class = QuietVolCanoMistralForCausalLM
    ...
    ########### quietstar config 属性############
    llama_config.max_thoughts=n_ahead + n_ahead_talk + 1
    llama_config.merged_talk_heads=merged_talk_heads
    llama_config.merged_lm_and_talk_heads=False
    llama_config.merged_lm_and_think_heads=True
    llama_config.use_concat_talk_head=True
    llama_config.use_shallow_think=True
    llama_config.use_shallow_talk=False
    llama_config.use_complex_think_head=False
    llama_config.use_complex_talk_head=True
    llama_config.use_weighted_talk_head=True
    ########### quietstar config 属性############
    ...
    model = model_class.from_pretrained(model_args.model_path, 
                                        config=llama_config, 
